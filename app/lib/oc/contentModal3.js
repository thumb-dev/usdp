...
function contentmodal3() {
    ...
    function template() {
        return [
            ...
            '<a ng-click="open3(500)" style="text-decoration: none;">',
            // replace the *Open Modal* below with your link name
            '<span class="modalName PDT-modalName"></span> {{\'How To Use\' | r | xlat}}',
            '</a>'
        ].join('');
    }
}

ContentModalCtrl3.$inject = ['$scope', '$modal'];
function ContentModalCtrl3($scope, $modal) {
    ...
    $scope.open3 = function (size) {
        ...
        function contentmodal3open() {
            return [
                ...
                '<iframe width="560" height="315" src="https://www.youtube.com/embed/Qa7W22llit0?autoplay=1&loop=1&playlist=GRonxog5mbw" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>',
                '</p>',
                '</div>',
                //Optional footer
    ...
    var ContentModalOpenCtrl3 = ['$scope', '$modalInstance', '$modal', 'item', function($scope, $modalInstance, $modal, item) {

        $scope.item = item; // this is the item passed in from the ContentModalCtrl3 resolve
